  ...
  <div>
    ...
    <div class="tool-bar" style="margin-bottom: 16px">
      <a-row>
        ...
        <a-col :span="12">
          <div class="btn-box">
            <a-button icon="rest" type="danger" @click="delSelect" :disabled="!hasSelected" v-action:governance-del>批量删除</a-button>
            <a-button icon="plus" type="primary" @click="addItem()" v-action:governance-add>添加</a-button>
    ...
    <div>
      ...
      >
        <div slot="sort" slot-scope="text">
          <div style="border-bottom: 1px solid #ddd; padding: 5px 0; text-indent: 12px;" v-if="!text.editable || !$store.getters.hasPermission('governance-edit')" @click="showEdit(text)">{{ text.sort }}</div>
          <div v-else>
        ...
        <div slot="status" slot-scope="text">
          <a-switch
            ...
            :disabled="!$store.getters.hasPermission('governance-edit')"
        ...
        <div class="action-box" slot="action" slot-scope="text">
          <a-tooltip placement="top" v-action:governance-edit>
            <template slot="title">
              <span>修改</span>
            </template>
            <a-button type="primary" icon="edit" @click="editItem(text)"></a-button>
          </a-tooltip>
          <a-tooltip placement="top" v-action:governance-del>
            <template slot="title">
              <span>删除</span>
            </template>
          ...
          <a-col :span="24">
            ...
            >
              <a-upload
                ...
                @change="handleChange"
              >
                <div v-if="imageUrl&&!loadingImg">已上传</div>
                <div v-if="loadingImg">
                  <a-icon :type="'loading'" />
...
export default {
  data () {
    return {
      ...
      actionUrl: this.$baseUrl + 'admin/article/upload_pdf' + (process.env.NODE_ENV === 'development' ? '?voucher=token' : '?voucher=session'),
      rules: {
  ...
  watch: {
    $route: function () {
      ...
      this.form.parent_code = 'governance'
    }
  },
  created () {
    // this.getCate()
  ...
  methods: {
    ...
    showEdit (item) {
      if (this.$store.getters.hasPermission('governance-edit')) {
        item.editable = true
        setTimeout(() => {
