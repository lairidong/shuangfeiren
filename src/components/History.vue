  ...
  <div>
    ...
    <div class="tool-bar" style="margin-bottom: 16px">
      <a-row>
        ...
        <a-col :span="12">
          <div class="btn-box">
            <a-button icon="rest" type="danger" @click="delSelect" :disabled="!hasSelected" v-action:history-del>批量删除</a-button>
            <a-button icon="plus" type="primary" @click="addItem()" v-action:history-add>添加</a-button>
    ...
    <div>
      ...
      >
        <div slot="sort" slot-scope="text">
          <div style="border-bottom: 1px solid #ddd; padding: 5px 0; text-indent: 12px;" v-if="!text.editable || !$store.getters.hasPermission('history-edit')" @click="showEdit(text)">{{ text.sort }}</div>
          <div v-else>
        ...
        <div slot="status" slot-scope="text">
          <a-switch
            ...
            :disabled="!$store.getters.hasPermission('history-edit')"
        ...
        <div class="action-box" slot="action" slot-scope="text">
          <a-tooltip placement="top" v-action:history-edit>
            <template slot="title">
              <span>修改</span>
            </template>
            <a-button type="primary" icon="edit" @click="editItem(text)"></a-button>
          </a-tooltip>
          <a-tooltip placement="top" v-action:history-del>
            <template slot="title">
              <span>删除</span>
            </template>
    ...
    >
      ...
      >
        <a-row :gutter="24">
          <a-col :span="12">
            <a-form-model-item ref="title" label="日期" prop="title">
              <a-input
                v-model="form.title"
                @blur="
                  () => {
...
const column1 = [
  { title: '日期', dataIndex: 'title', key: 'title' }
]
const columnSort = [
...
export default {
  data () {
    return {
      moment,
      locale,
      columns: column1.concat(columns2),
      pageSize: 10,
      currentPage: 1,
      total: 0,
      data: [],
  ...
  methods: {
    showEdit (item) {
      if (this.$store.getters.hasPermission('history-edit')) {
        item.editable = true
        setTimeout(() => {
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.status === 'S') {
            ...
            this.columns = columns.concat(columns2)
          }
        })
        .catch(error => {
          console.log(error)
