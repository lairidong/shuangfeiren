    ...
    <a-tabs defaultActiveKey="1">
      <a-tab-pane key="1" tab="基本信息">
        ...
        >
          ...
          <a-row :gutter="24">
            <a-col :span="24">
              <a-form-model-item ref="page_content" label="服务概述" prop="page_content">
                <a-textarea
      ...
      <a-tab-pane key="2" tab="主要服务">
        <div class="tool-bar" style="margin-bottom: 16px">
          <a-row>
            <a-col :span="24">
              <div class="btn-box">
                <a-button icon="plus" type="primary" @click="addAdvantageItem()" v-action:new-infrastructure-edit>添加</a-button>
              </div>
            </a-col>
          </a-row>
        </div>
        ...
        >
          ...
          <div slot="sort" slot-scope="text">
            <div style="border-bottom: 1px solid #ddd; padding: 5px 0; text-indent: 12px;" v-if="!text.editable || !$store.getters.hasPermission('new-infrastructure-edit')" @click="showEdit(text)">{{ text.sort }}</div>
            <div v-else>
          ...
          <div class="action-box" slot="action" slot-scope="text">
            <a-tooltip placement="top" v-action:new-infrastructure-edit>
              <template slot="title">
                <span>修改</span>
              </template>
      ...
      <a-tab-pane key="3" tab="商业案例">
        <div class="tool-bar" style="margin-bottom: 16px">
          <a-row>
            <a-col :span="24">
              <div class="btn-box">
                <a-button icon="plus" type="primary" @click="addCaseItem()" v-action:new-infrastructure-edit>添加</a-button>
              </div>
            </a-col>
          </a-row>
        </div>
        ...
        .then(res => {
          if (res.data.status === 'S') {
            ...
            if (this.page.attribute3 && this.page.attribute3.indexOf('[') > -1) {
              this.advantageData = JSON.parse(this.page.attribute3)
            } else {
              this.advantageData = []
            }
            if (this.page.attribute4 && this.page.attribute4.indexOf('[') > -1) {
              this.caseData = JSON.parse(this.page.attribute4)
            } else {
              this.caseData = []
            }
    ...
    handleOnSubmit () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.page.attribute3 = JSON.stringify(this.advantageData)
          this.page.attribute4 = JSON.stringify(this.caseData)
          this.page.code = 'new-infrastructure'
          this.$http
